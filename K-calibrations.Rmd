...
cal.curve <- read.csv("K-calibrations.csv")
...
renaming column 1 in the data set as R didn't like the format
```{r}

cal.curve <- cal.curve %>%
  rename(std.conc = std..concentration_ppm)
...
```{r}
  ...
  ggplot(data = cal.curve, aes(x = std.conc, y= std_reading)) +
  geom_point()+
...
mod1 <- lm(std_reading~std.conc, data = cal1)
...
intercept1 <- mod1$coefficients[1]

slope1 <- mod1$coefficients[2]
